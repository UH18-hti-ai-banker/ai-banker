...
API_OP_FUNDS <- function() {
  ...
  new("API", url=api_url, key=api_key, token=api_token)
}

API_OP_FUNDS_INFO <- function() {
...
setMethod("show", "API", function(object) {
  cat("URL:", object@url)
})

setGeneric("GET", function(object, endpoint) standardGeneric("GET"))
setMethod("GET", "API", function(object, endpoint = "") {
